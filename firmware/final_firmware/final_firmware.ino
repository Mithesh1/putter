...
#include <BLEUtils.h>
#include <BLE2902.h>
#define sensor_t adafruit_sensor_descriptor_t
...
static const int PIN_CAM_HREF = 11;
...
static const uint32_t CAMERA_XCLK_HZ = 20000000UL;
static const framesize_t BURST_FRAME_SIZE = FRAMESIZE_VGA;
static const int BURST_JPEG_QUALITY = 14;
static const int CAMERA_FB_COUNT = 3;
static const int INITIAL_SETTLE_FRAMES = 8;

static const uint32_t PRE_TRIGGER_MS = 350UL;
static const uint32_t POST_TRIGGER_MS = 1400UL;
static const size_t PREBUFFER_SLOTS = 8;
static const size_t MAX_BURST_FRAMES = 40;
static const size_t MAX_CAPTURE_BYTES = 5U * 1024U * 1024U;

static const char *AP_SSID = "OV5640-Burst";
static const int AP_CHANNEL = 1;
static const uint32_t CAMERA_TASK_STACK = 10240UL;
static const BaseType_t CAMERA_TASK_PRIORITY = 1;
static const BaseType_t CAMERA_TASK_CORE = 0;
...
bool cameraAutoTriggerArmed = false;
String lastError;
...
// ---------------------------------------------------------------------------
// Burst camera helpers and HTTP API
// ---------------------------------------------------------------------------

static const char *burstStateName(BurstState state) {
  switch (state) {
...
static void freeStoredFrame(StoredFrame &frame) {
...
static void clearStoredFrames(StoredFrame *frames, size_t count) {
  for (size_t i = 0; i < count; ++i) {
    freeStoredFrame(frames[i]);
  }
}

static bool copyBufferIntoFrame(StoredFrame &dst, const uint8_t *src, size_t len,
...
static bool copyCameraFrame(StoredFrame &dst, const camera_fb_t *fb,
                            uint32_t timestampMs) {
  return copyBufferIntoFrame(
      dst, fb->buf, fb->len, timestampMs, static_cast<uint16_t>(fb->width),
      static_cast<uint16_t>(fb->height));
}

static bool cloneStoredFrame(StoredFrame &dst, const StoredFrame &src) {
  if (src.data == nullptr || src.len == 0) {
    return false;
  }
  return copyBufferIntoFrame(dst, src.data, src.len, src.timestampMs, src.width,
                             src.height);
}

static void applyStableSensorDefaults(sensor_t *sensor) {
  if (sensor == nullptr) {
    return;
  }
...
static void setBurstError(const String &message) {
...
static void queueBurstTrigger(const char *source) {
  if (burstState == BurstState::Capturing) {
    return;
  }
  triggerPending = true;
  Serial.print("Burst trigger queued from ");
  Serial.println(source);
}

static void storeIntoPrebuffer(const camera_fb_t *fb, uint32_t timestampMs) {
...
static void updateBurstFps() {
  if (burstFrameCount < 2) {
    lastBurstFps = 0.0f;
    return;
  }

  const uint32_t firstMs = burstFrames[0].timestampMs;
  const uint32_t lastMs = burstFrames[burstFrameCount - 1].timestampMs;
  if (lastMs <= firstMs) {
    lastBurstFps = 0.0f;
    return;
  }

  lastBurstFps = (burstFrameCount - 1) * 1000.0f /
                 static_cast<float>(lastMs - firstMs);
}

static void finishBurstCapture(const String &reason) {
  ...
  lastError = "";
  Serial.printf(
      "Burst ready: %u frames (%u pre-trigger), %.1f fps estimate, %u bytes, reason=%s\n",
      static_cast<unsigned int>(burstFrameCount),
      static_cast<unsigned int>(burstPreFrameCount), lastBurstFps,
      static_cast<unsigned int>(burstTotalBytes), reason.c_str());
}

static void seedBurstFromPrebuffer(uint32_t triggerMs) {
  if (preValidCount == 0) {
    return;
  }

  const size_t oldestIndex = preValidCount == PREBUFFER_SLOTS ? preWriteIndex : 0;
  for (size_t i = 0; i < preValidCount && burstFrameCount < MAX_BURST_FRAMES; ++i) {
    const size_t idx = (oldestIndex + i) % PREBUFFER_SLOTS;
    const StoredFrame &candidate = prebuffer[idx];
    if (candidate.data == nullptr || candidate.len == 0) {
      continue;
    }
    if (triggerMs < candidate.timestampMs ||
        (triggerMs - candidate.timestampMs) > PRE_TRIGGER_MS) {
      continue;
    }
...
static bool storeBurstFrame(const camera_fb_t *fb, uint32_t timestampMs) {
...
static String buildStatusJson() {
  ...
  json += "\",";
  json += "\"putt_state\":\"";
  json += puttStateLabel(putt.state);
  json += "\",";
  json += "\"ble_connected\":";
  json += bleClientConnected ? "true" : "false";
  json += ",";
  json += "\"session_id\":";
  json += activeBleSessionId;
  json += "}";
  return json;
}
...
static void handleStatus() { server.send(200, "application/json", buildStatusJson()); }
...
static void handleTrigger() {
  if (burstState == BurstState::Capturing) {
    server.send(409, "application/json",
                "{\"accepted\":false,\"reason\":\"capture_in_progress\"}");
    return;
  }
  queueBurstTrigger("http");
...
static void handleFrame() {
  ...
  const int index = server.arg("id").toInt();
  if (index < 0 || static_cast<size_t>(index) >= burstFrameCount ||
      burstFrames[index].data == nullptr) {
...
static void configureCamera() {
  ...
  config.pin_sccb_sda = PIN_I2C_SDA;
  config.pin_sccb_scl = PIN_I2C_SCL;
...
static void captureOneCameraFrame() {
...
static void beginCameraApi() {
  ...
  xTaskCreatePinnedToCore(cameraTask, "camera_task", CAMERA_TASK_STACK, nullptr,
                          CAMERA_TASK_PRIORITY, &cameraTaskHandle,
                          CAMERA_TASK_CORE);
...
static void handleSerialArmCommand() {
  while (Serial.available() > 0) {
    ...
    Serial.println();
    if (incoming == ' ' || incoming == '\r' || incoming == '\n') {
      const uint32_t nowMs = millis();
      if ((nowMs - lastSerialArmMs) < 100UL) {
        continue;
      }
      lastSerialArmMs = nowMs;
      Serial.println("Serial trigger accepted");
      armReadyWindow();
    }
  }
}
  ...
  switch (putt.state) {
    ...
    case PUTT_BACKSTROKE:
      ...
      if (putt.forward_confirm_frames >= MOTION_CONFIRM_FRAMES) {
        ...
        if (cameraAutoTriggerArmed) {
          cameraAutoTriggerArmed = false;
          queueBurstTrigger("forward_stroke");
        }
...
void setup() {
  ...
  if (!psramFound()) {
    Serial.println("PSRAM is required for burst capture.");
    while (true) {
      delay(100);
    }
  }
