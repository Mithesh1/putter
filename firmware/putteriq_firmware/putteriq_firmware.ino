...
static const int PIN_PIEZO_2 = 7;
...
static const size_t PIEZO_CHANNEL_COUNT = 3;
...
static const float STROKE_START_GYRO_THRESHOLD_DPS = 15.0f;

static const uint32_t SETUP_STABLE_HOLD_MS = 700UL;
static const uint32_t FOLLOW_THROUGH_CAPTURE_MS = 250UL;

static const int PIEZO_IMPACT_THRESHOLD = 350;
...
static const size_t MAX_CAPTURE_IMU_SAMPLES = 512;
static const size_t MAX_CAPTURE_PIEZO_SAMPLES = 4096;
...
static void updatePiezoBaseline(PiezoState *piezo) {
  ...
  piezo->baseline_2 = (1.0f - alpha) * piezo->baseline_2 + alpha * piezo->raw_2;
...
static void readPiezos(PiezoState *piezo) {
  ...
  piezo->raw_2 = analogRead(PIN_PIEZO_2);
  piezo->raw_3 = analogRead(PIN_PIEZO_3);

  if (piezo->baseline_1 == 0.0f && piezo->baseline_2 == 0.0f &&
      piezo->baseline_3 == 0.0f) {
    piezo->baseline_1 = piezo->raw_1;
    piezo->baseline_2 = piezo->raw_2;
    piezo->baseline_3 = piezo->raw_3;
  }

  piezo->delta_1 = abs(piezo->raw_1 - (int)piezo->baseline_1);
  piezo->delta_2 = abs(piezo->raw_2 - (int)piezo->baseline_2);
  piezo->delta_3 = abs(piezo->raw_3 - (int)piezo->baseline_3);

  piezo->peak_delta = max(piezo->delta_1, max(piezo->delta_2, piezo->delta_3));
...
static void appendPiezoSample() {
  ...
  strokeCapture.piezo_data[offset + 1] = clampToInt16(latestPiezo.raw_2);
  strokeCapture.piezo_data[offset + 2] = clampToInt16(latestPiezo.raw_3);
...
static void appendImuSample() {
  ...
  strokeCapture.imu_data[offset + 0] =
      clampToInt16((int32_t)lroundf(latestImu.accel_x_g * ACCEL_SCALE));
  strokeCapture.imu_data[offset + 1] =
      clampToInt16((int32_t)lroundf(latestImu.accel_y_g * ACCEL_SCALE));
  strokeCapture.imu_data[offset + 2] =
      clampToInt16((int32_t)lroundf(latestImu.accel_z_g * ACCEL_SCALE));
  strokeCapture.imu_data[offset + 3] =
      clampToInt16((int32_t)lroundf(latestImu.gyro_x_dps * GYRO_SCALE));
  strokeCapture.imu_data[offset + 4] =
      clampToInt16((int32_t)lroundf(latestImu.gyro_y_dps * GYRO_SCALE));
  strokeCapture.imu_data[offset + 5] =
      clampToInt16((int32_t)lroundf(latestImu.gyro_z_dps * GYRO_SCALE));
  strokeCapture.imu_data[offset + 6] =
      clampToInt16((int32_t)lroundf(latestImu.quat_i * QUATERNION_SCALE));
  strokeCapture.imu_data[offset + 7] =
      clampToInt16((int32_t)lroundf(latestImu.quat_j * QUATERNION_SCALE));
  strokeCapture.imu_data[offset + 8] =
      clampToInt16((int32_t)lroundf(latestImu.quat_k * QUATERNION_SCALE));
  strokeCapture.imu_data[offset + 9] =
      clampToInt16((int32_t)lroundf(latestImu.quat_r * QUATERNION_SCALE));
...
static void sendFragment(uint32_t strokeId, uint16_t fragmentIndex,
                         uint16_t fragmentCount, const uint8_t *payload,
                         uint16_t payloadLength) {
  writeUint16LE(fragmentBuffer, 0, FRAGMENT_MAGIC);
  fragmentBuffer[2] = PROTOCOL_VERSION;
  fragmentBuffer[3] = MESSAGE_TYPE_STROKE;
  writeUint32LE(fragmentBuffer, 4, strokeId);
...
static void sendStrokeCaptureOverBle() {
  ...
  for (uint16_t fragmentIndex = 0; fragmentIndex < fragmentCount; fragmentIndex++) {
    ...
    sendFragment(packetId, fragmentIndex, fragmentCount,
...
static void sendWaitingForPuttOverBle() {
  ...
  const int len = snprintf((char *)rawPacketBuffer,
                           ...
                           "Waiting for putt ts=%lu state=%d setup=%u",
                           (unsigned long)millis(),
                           (int)putt.state,
                           putt.setup_found ? 1 : 0);
  if (len <= 0) {
    return;
  }
...
static bool strokeHasStarted() {
  if (!latestImu.gyro_valid) {
    return false;
  }
  return fabsf(strokeMotionRateDps(latestImu)) > STROKE_START_GYRO_THRESHOLD_DPS;
}

static void maybeCaptureSetup() {
  static uint32_t stableStartMs = 0UL;
  ...
  if (stableStartMs == 0UL) {
    stableStartMs = millis();
    return;
  }

  if ((millis() - stableStartMs) >= SETUP_STABLE_HOLD_MS) {
    putt.setup_found = true;
    putt.setup_locked_ms = millis();
    putt.setup_orientation.yaw_deg = latestImu.yaw_deg;
    putt.setup_orientation.pitch_deg = latestImu.pitch_deg;
    putt.setup_orientation.roll_deg = latestImu.roll_deg;
  }
}
...
static void updatePuttStateMachine() {
  ...
  if (putt.setup_found && latestImu.orientation_valid) {
    const float face_delta =
        wrapDegrees180(faceRotationDeg(latestImu) - putt.setup_orientation.yaw_deg);
  ...
  if (latestImu.gyro_valid) {
    putt.peak_stroke_rate_dps =
        max(putt.peak_stroke_rate_dps, fabsf(strokeMotionRateDps(latestImu)));
  }

  switch (putt.state) {
    case PUTT_IDLE:
      if (putt.setup_found && strokeHasStarted()) {
        putt.state = PUTT_BACKSTROKE;
        putt.stroke_start_ms = now_ms;
        beginStrokeCapture();
      }
      break;

    case PUTT_BACKSTROKE:
      if (latestImu.gyro_valid && strokeMotionRateDps(latestImu) < 0.0f) {
        putt.state = PUTT_FORWARD_STROKE;
      }
      if (latestPiezo.impact_detected) {
        putt.state = PUTT_IMPACT;
        putt.impact_ms = now_ms;
        strokeCapture.impact_time_ms = now_ms;
      }
      break;
    ...
    case PUTT_IMPACT:
      putt.follow_through_end_ms = now_ms + FOLLOW_THROUGH_CAPTURE_MS;
      putt.state = PUTT_FOLLOW_THROUGH;
      break;

    case PUTT_FOLLOW_THROUGH:
      if (now_ms >= putt.follow_through_end_ms) {
        putt.state = PUTT_RESULT_READY;
      }
      break;
...
static void printCompactImuLine() {
  ...
  Serial.print(" | setup=");
  Serial.print(putt.setup_found ? 1 : 0);
  Serial.print(" | face=");
...
void loop() {
  ...
  if (putt.state == PUTT_IDLE && !latestPiezo.impact_detected) {
