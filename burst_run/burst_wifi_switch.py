...
def candidate_step_diameters(a, b):
    ...
    return math.hypot(float(a[0]) - float(b[0]), float(a[1]) - float(b[1])) / (avg_radius * 2.0)
...
def predicted_candidate(prev_candidate, prev_prev_candidate):
    if prev_candidate is None or prev_prev_candidate is None:
        return None
    return (
        float(prev_candidate[0]) + (float(prev_candidate[0]) - float(prev_prev_candidate[0])),
        float(prev_candidate[1]) + (float(prev_candidate[1]) - float(prev_prev_candidate[1])),
...
def snapshot_raw_results(results):
    for result in results:
        ...
        result["raw_tracking_mode"] = result.get("tracking_mode", "detect")
...
def blend_signature(current_signature, new_signature):
    ...
    return {
        key: (1.0 - SIGNATURE_BLEND) * current_signature[key] + SIGNATURE_BLEND * new_signature[key]
...
def apply_stabilized_candidate(result, candidate, burst_mode, keep_raw_line=False):
    ...
    if burst_mode == "raw":
        result["tracking_mode"] = result.get("raw_tracking_mode", result.get("tracking_mode", "detect"))
    else:
...
def recompute_rotation_metrics(metrics, fps):
    ...
    for index, result in enumerate(results):
        if result.get("ball_found") and result.get("line_found") and result.get("angle") is not None:
...
def stabilize_burst_tracking(metrics, frames, manifest, fps):
    ...
    for index, result in enumerate(results):
        ...
        raw_signature = compute_ball_signature(frames[index], raw_candidate) if raw_candidate is not None else None
        radius_ok_to_anchor = candidate_radius_change(raw_candidate, anchor_candidate) <= MAX_RADIUS_CHANGE_RATIO
        ...
        if not released:
            if raw_candidate is not None and radius_ok_to_anchor:
                ...
                if move_from_anchor >= RELEASE_MIN_STEP_DIAMETERS:
                    if not release_candidates:
                        ...
                        release_step = candidate_step_diameters(raw_candidate, release_candidates[-1])
            ...
            can_release_now = index >= (max(pre_frame_count, 0) + POST_TRIGGER_ANCHOR_HOLD_FRAMES - 1)
            if (
                can_release_now
                and raw_candidate is not None
                and len(release_candidates) >= RELEASE_CONFIRM_FRAMES
                and signature_distance(raw_signature, stable_signature) <= MAX_SIGNATURE_DISTANCE
            ):
                released = True
                ...
                apply_stabilized_candidate(result, anchor_candidate, "burst_anchor", keep_raw_line=False)
            continue

        accept_raw = False
        if raw_candidate is not None:
            radius_ok = candidate_radius_change(raw_candidate, stable_prev) <= MAX_RADIUS_CHANGE_RATIO
            step_ok = candidate_step_diameters(raw_candidate, stable_prev) <= MAX_TRACK_STEP_DIAMETERS
            predicted = predicted_candidate(stable_prev, stable_prev2)
            prediction_ok = (
                predicted is not None
                and candidate_step_diameters(raw_candidate, predicted) <= MAX_PREDICTION_ERROR_DIAMETERS
            )
            signature_ok = signature_distance(raw_signature, stable_signature) <= MAX_SIGNATURE_DISTANCE
    ...
    if result.get("velocity_mph") is not None:
        cv2.putText(
            ...
            "speed {:.2f} mph | {:.2f} ball/s".format(
                result["velocity_mph"], result["velocity_ball_diam_s"]
            ),
            (12, 80),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.55,
...
def _pack_metrics(metrics):
    # 8 little-endian float32s + uint8 + uint16 + float32 = 39 bytes
    return struct.pack(
        "<8fBHf",
...
def run_capture_once(calibrate=False):
    ...
    # Push results to iPad via BLE before opening review window.
